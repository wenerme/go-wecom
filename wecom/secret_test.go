...
func TestTokenCache(t *testing.T) {
	...
	// no expire
	{
		...
		token, err = tc.Refresh(&GenericToken{
			...
			return &GenericToken{Secret: "1"}, nil
		})
		assert.NoError(t, err)
		...
		token, err = tc.Refresh(&GenericToken{
			...
			return &GenericToken{Secret: "2"}, nil
		})
		assert.NoError(t, err)
		assert.Equal(t, "1", token)
		...
		token, err = tc.Refresh(&GenericToken{
			...
			return &GenericToken{Secret: "3"}, nil
		})
		assert.NoError(t, err)
		assert.Equal(t, "2", token)
		// deps
		...
		token, err = tc.Refresh(&GenericToken{
			...
			return &GenericToken{Secret: "4"}, nil
		})
		assert.NoError(t, err)
		assert.Equal(t, "3", token)
		// no deps
	...
	// with expire
	{
		...
		token, err := tc.Refresh(&GenericToken{
			...
			return &GenericToken{Secret: "1", ExpiresIn: 100}, nil
		})
		assert.NoError(t, err)
		assert.Equal(t, "1", token)
		...
		token, err = tc.Refresh(&GenericToken{
			...
			return &GenericToken{Secret: "2", ExpiresIn: 100}, nil
		})
		assert.NoError(t, err)
		assert.Equal(t, "1", token)
		...
		token, err = tc.Refresh(&GenericToken{
			...
			return &GenericToken{Secret: "3", ExpiresIn: 100}, nil
		})
		assert.NoError(t, err)
		assert.Equal(t, "2", token)
		...
		_, err = tc.Refresh(&GenericToken{
			...
			return &GenericToken{Secret: "3", ExpiresIn: 100}, errors.New("failed")
		})
		assert.Error(t, err)
		//
		token, err = tc.Refresh(&GenericToken{
